// https://github.com/vasturiano/timelines-chart Version 1.0.2. Copyright 2016 Vasco Asturiano.
...
selection.prototype.textFitToBox = function(w,h,passes) {
    passes = passes||3;
...
selection.prototype.textAbbreviateToFit = function(maxW) {
...
selection.prototype.addGradient = function(colorScale, angle) {
...
selection.prototype.addDropShadow = function() {
...
selection.prototype.appendOrdinalColorLegend = function(w, h, scale, label) {
...
selection.prototype.appendLinearColorLegend = function(w, h, scale, label) {
...
selection.prototype.appendColorLegend = function(x, y, w, h, scale, label) {
...
selection.prototype.appendSvgThrobber = function(x, y, r, color$$1, duration, angleFull) {
    ...
    function genDonutSlice(cx, cy, r, thickness, startAngle, endAngle) {
        ...
        path$$1.push("M" + p[0].join());
        path$$1.push("A" + [outerR,outerR,0,largeArc,1,p[1]].join());
        path$$1.push("L" + p[2].join());
        path$$1.push("A" + [innerR,innerR,0,largeArc,0,p[3]].join());
        path$$1.push("z");
    ...
    r = r||8;
    color$$1 = color$$1||'darkblue';
    duration = duration||0.7;
    angleFull = angleFull||120;

    var thickness = r/3;

    var path$$1 = this.append('path')
        .attr('d', genDonutSlice(x, y, r, thickness, 0, angleFull))
        .attr('fill', color$$1);

    path$$1.append('animateTransform')
...
selection.prototype.appendImage = function(imgUrl, x, y, maxW, maxH, svgAlign) {
...
var TimeOverview = function(options, callback, context){
    var margins, width, hideIfLessThanSeconds, externalMoveEvent,
        height, brush$$1, xAxis, svg, groupOverview, $this, dom, labels, verticalLabels, format$$1;
    ...
    this._afterInteraction = function(){
        ...
        if (externalMoveEvent) {
            ...
            var selection$$1 = event.selection.map(xAxis.invert);
            callback.call(context, selection$$1[0], selection$$1[1]);
        }
    };
    ...
    this.render = function(domainRange, currentSelection){
        ...
        groupOverview = svg.append("g")
            .attr("class", "brush")
            .call(brush$$1)
            .call(brush$$1.move, currentSelection.map(xAxis));
...
var timelines = function() {
    ...
    function initStatic() {
        ...
        function buildDomStructure () {
            ...
            function invertOrdinal(val, cmpFunc) {
                ...
                if (range$$1.length === 2 && domain.length !== 2) {
                    // Special case, interpolate range vals
                    range$$1 = range(range$$1[0], range$$1[1], (range$$1[1] - range$$1[0]) / domain.length);
                }

                var bias = range$$1[0];
                for (var i = 0, len = range$$1.length; i < len; i++) {
                    if (cmpFunc(range$$1[i] + bias, val)) {
                        return domain[Math.round(i * domain.length / range$$1.length)];
                    }
                }
        ...
        function addTooltips() {
            ...
            env.segmentTooltip = index$5()
                ...
                .html(function(d) {
                    ...
                    var dateFormat = timeFormat("%Y-%m-%d %H:%M:%S");
        ...
        function addZoomSelection() {
            env.graph.on("mousedown", function() {
                ...
                if (mouse(e)[0]<0 || mouse(e)[0]>env.graphW || mouse(e)[1]<0 || mouse(e)[1]>env.graphH)
                    return;
                ...
                select(window)
                    .on("mousemove.zoomRect", function() {
                        ...
                        var newCoords = [
                            Math.max(0, Math.min(env.graphW, mouse(e)[0])),
                            Math.max(0, Math.min(env.graphH, mouse(e)[1]))
                        ];
                        ...
                        env.svg.dispatch('zoomScent', { detail: {
                            zoomX: [startCoords[0], newCoords[0]].sort(ascending).map(env.xScale.invert),
                    ...
                    .on("mouseup.zoomRect", function() {
                        ...
                        var newDomainX = [startCoords[0], endCoords[0]].sort(ascending).map(env.xScale.invert);
        ...
        function setEvents() {
            ...
            env.svg.on('resetZoom', function() {
                ...
                var newZoomX = env.enableOverview
                    ...
                    :[
                        min(env.flatData, function(d) { return d.timeRange[0]; }),
                        max(env.flatData, function(d) { return d.timeRange[1]; })
                    ],
                    newZoomY = [null, null];
    ...
    function drawNewData(data, keepGraphStructure) {
        ...
        if (keepGraphStructure) {
            ...
            env.zoomX = [
                min(env.completeFlatData, function(d) { return d.timeRange[0]; }),
                max(env.completeFlatData, function(d) { return d.timeRange[1]; })
            ];
    ...
    function draw() {
        ...
        function setupHeights() {
            env.graphH = min([env.nLines*env.lineMaxHeight, env.maxHeight-env.margin.top-env.margin.bottom]);
        ...
        function adjustXScale() {
            ...
            env.zoomX[0] = env.zoomX[0] || min(env.flatData, function(d) { return d.timeRange[0]; });
            env.zoomX[1] = env.zoomX[1] || max(env.flatData, function(d) { return d.timeRange[1]; });
        ...
        function renderAxises() {
            ...
            */
            ...
            // Grp
            var minHeight = min(env.grpScale.range(), function (d,i) {
                    ...
                    select(this)
                        ...
                        .transition().duration(70)
                            ...
                            .attr('width', function (d) {
                                return max([1, env.xScale(d.timeRange[1])-env.xScale(d.timeRange[0])])+hoverEnlarge;
                            })
                            .attr('y', function (d) {
                ...
                .on("mouseout", function() {
                    select(this)
                        .transition().duration(250)
                            ...
                            .attr('width', function (d) {
                                return max([1, env.xScale(d.timeRange[1])-env.xScale(d.timeRange[0])]);
                            })
                            .attr('y', function (d) {
    ...
    chart.sortChrono = function(asc) {
        ...
        function buildIdx(accessFunction) {
            ...
            for (var i= 0, len=env.completeFlatData.length; i<len; i++ ) {
                ...
                idx[key] = [
                    min(itmList, function(d) { return d.timeRange[0]}),
                    max(itmList, function(d) { return d.timeRange[1]})
                ];
            }
            return idx;
        }
    ...
    chart.getSvg = function() {
        return select(env.svg.node().parentNode).html();
    };
